/*
 * main.cpp
 * modified from code provided by Stephane Audic:
* Copyright Stephane Audic 2003
With code included from Numerical Recipes in C 


 modification by albert cheng 2010
  ...
  while( argcount < argc ){
    ...
    if( !strcmp( argv[argcount] , "-xvalue")){
	 ...
	 from the samples  is different */
      ...
      argcount += 2 ;
    } else if(!strcmp(argv[argcount],"-interactive")){
    	argcount++;
    	interactive=true;
    }
    else {
  ...
  vector<string> splits;
   if(interactive)
   {
	   while(!cin.eof())
	   {
		   ...
		   x=StringUtil::atoi(splits[0]);
		   y=StringUtil::atoi(splits[1]);
		   n1=StringUtil::atoi(splits[2]);
		   n2=StringUtil::atoi(splits[3]);
		   ...
		   ymax =  WNMAX ;
			...
			fprintf( stdout , "%d\t%d\t%d\t%d\t%g\t%g\n" ,x,y,n1,n2, thisproba,thisproba2 ) ;
